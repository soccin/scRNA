...
get_qc_tables <- function(md,filters) {
    ...
    tbl3a=md %>% group_by(SampleID) %>% summarize(Fail.Features=mean(nFeature_RNA<=MIN_FEATURE_RNA),Fail.Count=mean(nCount_RNA<=MIN_NCOUNT_RNA),Fail.Mito=mean(percent.mt>=PCT_MITO))
