...
qcSamples <- function(so) {
    ...
    ptb=list()
    ptb[[len(ptb)+1]]=ggplot()+theme_void()+annotation_custom(tableGrob(mutate_if(tbl0,is.numeric,~round(.,2)),rows=NULL))
    ptb[[len(ptb)+1]]=ggplot()+theme_void()+annotation_custom(tableGrob(mutate_if(tbl,is.numeric,~round(.,2)),rows=NULL))
    tt=tblb %>% mutate(PCT.Fail=round(PCT.Fail*100,2))
    ptb[[len(ptb)+1]]=ggplot()+theme_void()+annotation_custom(tableGrob(mutate_if(tt,is.numeric,~round(.,2)),rows=NULL))
    tt=tbl3a %>% mutate_if(is.numeric,\(x)x*100)
    ptb[[len(ptb)+1]]=ggplot()+theme_void()+annotation_custom(tableGrob(mutate_if(tt,is.numeric,~round(.,2)),rows=NULL))
    ptb[[len(ptb)+1]]=ggplot()+theme_void()+annotation_custom(tableGrob(mutate_if(tbl3b,is.numeric,~round(.,2)),rows=NULL))

    pq[[len(pq)+1]]=(ptb[[1]]+ptb[[2]]+ptb[[3]])/(ptb[[4]]+ptb[[5]])

    list(plts=pq, stats=list(tbl0,tbl,tblb,tbl3a,tbl3b))
