...
for (ii in seq(d10X)) {
    print(ii)
    d10X.int[[ii]] <- SCTransform(d10X[[ii]], vars.to.regress = c('S.Score', 'G2M.Score'), verbose = T)
}

cat("\n**************************************************\n")
cat("\nSelect & Prep Integration\n")
...
d10X.integrate.orig=d10X.integrate

save.image(cc("CHECKPOINT-103",DATE(),".RData"),compress=T)
...
so@meta.data$shortName=gsub("K27M_p53DD_D842V_","",so@meta.data$orig.ident)

pg0=list()
pg0[[len(pg0)+1]]=DimPlot(so,reduction="umap",group.by="shortName") + scale_color_brewer(palette="Paired")
pg0[[len(pg0)+1]]=DimPlot(so,reduction="umap",group.by="Phase")
...
s1=d10X.integrate.orig

s1=CellCycleScoring(s1,
                    s.features=cellCycle.genes$s.genes,
                    g2m.features=cellCycle.genes$g2m.genes,
                    set.ident=T
                    )
...
if(maxClusters>33) {
    save.image(cc("CHECKPOINT-220",DATE(),".RData"),compress=T)
    stop("\n\nTOO MANY CLUSTERS\n\n")
}
...
save.image(cc("CHECKPOINT-128",DATE(),".RData"),compress=T)
